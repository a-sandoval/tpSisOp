...
int conexionCPU(t_config* config_clienteCPU) {
   ...
   char* ip; 
   char* claveHandshake; 
   obtenerDeConfiguracionComoClienteCPU(config_clienteCPU, &puertoConexionACPU,&ip,&claveHandshake);    
   int conexionACPU = crear_conexion(ip, puertoConexionACPU); 
   handshake(claveHandshake,conexionACPU); 
   return conexionACPU; 
}

void obtenerDeConfiguracionComoClienteCPU(t_config* config_clienteCPU, char** puerto, char** ip, char** claveHandshake) {
...
void terminar_programaCPU(int conexion, t_log* logger){
	log_destroy(logger);
